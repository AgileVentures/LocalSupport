...
Then /^I should see "([^"]*?)", "([^"]*?)" and "([^"]*?)" in the map centered on local organizations$/ do |name1, name2, name3|
  ...
  page.should have_xpath "//script[contains(.,'Gmaps.map.map_options.auto_adjust = false')]", :visible => false
  page.should have_xpath "//script[contains(.,'Gmaps.map.map_options.auto_zoom = true')]", :visible => false
  page.should have_xpath "//script[contains(.,'Gmaps.map.map_options.center_latitude = 51.5978')]", :visible => false
  page.should have_xpath "//script[contains(.,'Gmaps.map.map_options.center_longitude = -0.337')]", :visible => false
  page.should have_xpath "//script[contains(.,'Gmaps.map.map_options.zoom = 12')]", :visible => false
  page.should have_xpath "//script[contains(.,'Gmaps.map.map_options.auto_adjust = false')]", :visible => false
end
...
def check_map(names)
  names.each do |name|
    page.should have_xpath "//script[contains(.,\"#{name}\")]", :visible => false
