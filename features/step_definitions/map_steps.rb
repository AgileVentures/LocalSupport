Then /^I should see hyperlinks for "(.*?)", "(.*?)" and "(.*?)" in the map$/ do |org1, org2, org3|
  ...
  [org1,org2,org3].each do |org|
    match = page.html.match /\\u003ca href=\\\"\/organisations\/#{Regexp.quote(org.id.to_s)}\\\"\\u003e#{Regexp.quote(org.name)}\\u003c\/a\\u003e/
...
Then /^the coordinates for "(.*?)" and "(.*?)" should( not)? be the same/ do | org1_name, org2_name, negation|
  ...
  matches = page.html.match %Q<{\\"zindex\\":(?:1|0)?,\\"description\\":\\"[^}]*#{org1_name}[^}]*\\",\\"lat\\":((?:-|)\\d+\.\\d+),\\"lng\\":((?:-|)\\d+\.\\d+)}>
  org1_lat = matches[1]
  org1_lng = matches[2]
  matches = page.html.match %Q<{\\"zindex\\":(?:1|0)?,\\"description\\":\\"[^}]*#{org2_name}[^}]*\\",\\"lat\\":((?:-|)\\d+\.\\d+),\\"lng\\":((?:-|)\\d+\.\\d+)}>
  org2_lat = matches[1]
  org2_lng = matches[2]
