    ...
    context 'PATCH #update' do
      ...
      context 'when email is invalid' do
        ...
        context 'when there is no email and organisation is not accepted' do
          before(:each) do
            result = AcceptProposedOrganisation::Response.new(
                AcceptProposedOrganisation::Response::NO_EMAIL, nil, nil
            )
            result.not_accepted_org = orphan_org
            allow_any_instance_of(AcceptProposedOrganisation)
                .to receive(:run).and_return(result)
          end
          
          it 'shows error flash with a certain message' do
            patch :update, params: {id: orphan_org.id}
            message = 'No invitation email was sent because'\
              ' no email is associated with the organisation'
            expect(flash[:error]).to have_text(message)
          end
        ...
        context 'when there is invalid email and organisation is not accepted' do
          before(:each) do
            result = AcceptProposedOrganisation::Response.new(
                AcceptProposedOrganisation::Response::INVALID_EMAIL, nil, nil
            )
            result.not_accepted_org = orphan_org
            allow_any_instance_of(AcceptProposedOrganisation)
                .to receive(:run).and_return(result)
