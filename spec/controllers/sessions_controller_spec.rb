...
describe SessionsController, type: :controller do
  describe 'POST create' do
    ...
    it 'redirects to home page after superadmin logs-in' do
      FactoryBot.build(
          :user,
          email: 'example@example.com', password: 'pppppppp', superadmin: true
      ).save!
      post :create, params: {
          'user' => {'email' => 'example@example.com', 'password' => 'pppppppp'}
      }
    ...
    it 'renders sign in page after someone fails to log-in with non-existent account' do
      post :create, params: {
          'user' => {'email' => 'example@example.com', 'password' => '12345'}
      }
    ...
    it 'displays warning flash after someone fails to log-in with non-existent account' do
      ...
      message = "I'm sorry, you are not authorized to login to the system."
      expect(flash[:alert]).to have_content message
    end
